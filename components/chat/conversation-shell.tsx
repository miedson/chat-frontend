...
export function ConversationShell({ conversationId }: { conversationId: string }) {
  ...
  const canReply = useMemo(() => conversation?.status !== 'open', [conversation?.status])
  ...
  async function loadConversation() {
    ...
    if (found.status === 'open' || !found.assignedTo) {
      setError('Conversa nao esta atribuida para atendimento.')
      return false
    }
  ...
  async function loadMessages(silent = false) {
    const response = await apiFetch(`/api/conversations/${conversationId}/messages?limit=100`)
    const payload = (await response.json().catch(() => null)) as
      | Message[]
      | { message?: string }
      | null
  ...
  async function handleSend(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await apiFetch(`/api/conversations/${conversationId}/messages`, {
      ...
      body: JSON.stringify({ content: messageInput.trim(), type: 'outgoing' }),
    })
  ...
  return (
    <main className="conversation-layout">
      <header className="conversation-header">
        <div>
          <span className="chat-kicker">ATENDIMENTO</span>
        ...
        <div className="conversation-header-actions">
          ...
          {conversation?.status === 'resolved' ? (
            <button
              ...
              disabled={changingStatus}
            >
              {changingStatus ? 'Atualizando...' : 'Reabrir atendimento'}
            </button>
          ) : null}
